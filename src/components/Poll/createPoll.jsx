...
import { httpPost1, httpGet1, httpPatch,httpPatch1, httpDelete } from '../../actions/data.action';
...
export default function CreatePoll(props) {
  ...
  const handlePollOptions=(type,deleteData,data = "")=>{
    if (type === "add") {
      ...
      if(question_id !== ""){
        let option = ques_options.filter((que)=>que.question.id === question_id).map((que)=>{
          return que.options;
        });
        let opt = [...option[0]].filter((opt)=>opt.option === data)
        deleteHandler(`/opinion_poll/${id}/question/${question_id}/option/${opt[0].id}`)
      }
  ...
  const savePoll = async (event)=>{
      if(PollOptions.length === 0 || pollQuestion.trim().length === 0){
        return NotificationManager.error("Please enter question and at least an option");
      }
      try{
        showLoader()
        let fd = {
        ...
        if(question_id !== ''){
          let res = await httpPatch1(`/opinion_poll/${id}/question/${question_id}`,fd);
          setPollOptions([]);
          setpollQuestion("");
          setQuestionId("");
          getQuestions();
                      ...
                      <div className="createPollForm-col-one-child">
                          <form>
                              <div className="poll-input-wrap">
                                  ...
                                  <textarea onChange={(e)=>setpollQuestion(e.target.value)} required placeholder="Type your first question here"  name="" id="" cols="30" rows="10" value={pollQuestion}></textarea>
                              </div>
                                  ...
                                  <div className="poll-tags">
                                  {
                                      ...
                                      return (
                                        <div className="poll-options-drow">
                                            ...
                                            <img src="/remove.png" onClick={()=>handlePollOptions("remove",index,data)} />
                    ...
                    <div className="poll-tags">
                      {
                        ques_options.map((que,index)=>(
                          <div className="ml-4 mr-4 mt-4">
                            <span className="fa fa-remove pull-right text-danger pull-action" onClick={()=>removeData(que)}></span>
                            <span className="fa fa-pencil pull-right text-success mr-3 pull-action" onClick={()=>editData(que)}></span>
                            <Link to={`/answers/${que.question.id}`}
