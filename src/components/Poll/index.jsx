...
export default function Index(props) {
  const getPolls = async () => {
    try {
      ...
      let query =
        department.length > 0
          ? `opinion_polls/branch/${branch}/department/${department}`
          : branch.length > 0
          ? `opinion_polls/branch/${branch}`
          : `general_opinion_polls`;
      console.log("query>>", query);
      let res = await httpGet1(query);
      console.log("res>>", res);
      setPoll({});
      setPolls(
        query === "general_opinion_polls"
          ? res.data.generalOpinionPolls
          : department.length > 0
          ? res.departmentOpinionPolls
          : res.branchOpinionPolls
      );
                ...
                <div className="col-md-7">
                  <div className="mb-5">
                    ...
                    <div className="poll-filter-container">
                      <select
                        className="poll-filter form-control"
                    ...
                    <div className="poll-filter-container">
                      <button className="btn btn-default" onClick={getPolls}>
